// components/TransferForm.tsx
    import React, { useState, useEffect } from "react";
    import { Building2, CreditCard, DollarSign, FileText, Send, RefreshCw, CheckCircle2 } from "lucide-react";
    import { Account } from '@/src/types/account';
    import { TransferData } from '@/src/types/transaction';

    interface TransferFormProps {
        fromAccounts: Account[];
        selectedAccount?: Account;
        initialData?: {
            accountIBAN?: string;
            firstName?: string;
            secondName?: string;
            lastName?: string;
        };
        currencySymbols: Record<string, string>;
        isLoading: boolean;
        onSubmit: (data: TransferData) => void;
        onClose: () => void;
        showAlert: (type: "success" | "error" | "warning" | "info", title: string, message: string) => void;
    }

    export default function TransferForm({
                                             fromAccounts,
                                             selectedAccount,
                                             initialData,
                                             currencySymbols,
                                             isLoading,
                                             onSubmit,
                                             onClose,
                                             showAlert
                                         }: TransferFormProps) {

        const [formData, setFormData] = useState<TransferData>({
            fromIBAN: selectedAccount?.iban || "",
            toIBAN: "",
            amount: "",
            description: "",
            toFirstName: "",
            toSecondName: undefined,
            toLastName: "",
        });

        const [recipientNameConfirm, setRecipientNameConfirm] = useState("");
        const [recipientName, setRecipientName] = useState<string | null>(null);
        const [ibanLoading, setIbanLoading] = useState(false);
        const [nameMatch, setNameMatch] = useState<boolean | null>(null);

        const currentAccount = fromAccounts.find(a => a.iban === formData.fromIBAN);

        useEffect(() => {
            if (initialData) {
                const fullName = [initialData.firstName, initialData.secondName, initialData.lastName]
                    .filter(Boolean)
                    .join(" ");

                setFormData(prev => ({
                    ...prev,
                    toIBAN: initialData.accountIBAN || "",
                    toFirstName: initialData.firstName || "",
                    toSecondName: initialData.secondName,
                    toLastName: initialData.lastName || "",
                }));
                setRecipientNameConfirm(fullName);
                setRecipientName(fullName);
                setNameMatch(true);
            }
        }, [initialData]);

        const maskName = (name: string) => {
            return name.split(" ").map(p => p.length > 2 ? p[0] + "*".repeat(p.length - 1) : p[0] + "*").join(" ");
        };

        const fetchRecipientName = async (iban: string) => {
            if (iban.replace(/\s/g, "").length < 26) {
                setRecipientName(null);
                setFormData(prev => ({ ...prev, toFirstName: "", toSecondName: undefined, toLastName: "" }));
                return;
            }
            setIbanLoading(true);
            try {
                const res = await fetch(`/api/account/get-name-by-iban?iban=${encodeURIComponent(iban)}`);
                const data = await res.json();
                if (res.ok && data.firstName) {
                    const fullName = [data.firstName, data.secondName, data.lastName].filter(Boolean).join(" ");
                    setRecipientName(fullName);
                    setFormData(prev => ({
                        ...prev,
                        toFirstName: data.firstName || "",
                        toSecondName: data.secondName,
                        toLastName: data.lastName || "",
                    }));
                    showAlert("info", "Alıcı Bulundu", "IBAN sahibinin adı getirildi.");
                } else {
                    setRecipientName(null);
                    setFormData(prev => ({ ...prev, toFirstName: "", toSecondName: undefined, toLastName: "" }));
                    showAlert("warning", "Uyarı", "Kullanıcı bulunamadı.");
                }
            } catch {
                showAlert("error", "Hata", "Sorgulama başarısız.");
            } finally {
                setIbanLoading(false);
            }
        };

        const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
            const { name, value } = e.target;

            if (name === "fromIBAN") {
                setFormData(prev => ({ ...prev, fromIBAN: value }));
            } else if (name === "toIBAN") {
                setFormData(prev => ({ ...prev, toIBAN: value }));
                if (value.replace(/\s/g, "").length === 26) {
                    fetchRecipientName(value);
                } else {
                    setRecipientName(null);
                    setNameMatch(null);
                }
            } else if (name === "amount" || name === "description") {
                setFormData(prev => ({ ...prev, [name]: value }));
            } else if (name === "recipientNameConfirm") {
                setRecipientNameConfirm(value);
                if (recipientName) {
                    const isMatch = value.trim().toUpperCase() === recipientName.trim().toUpperCase();
                    setNameMatch(isMatch);
                }
            }
        };

        const handleAccountChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
            const selectedIban = e.target.value;
            setFormData(prev => ({ ...prev, fromIBAN: selectedIban }));
        };

        const handleSubmit = (e: React.FormEvent) => {
            e.preventDefault();

            if (!formData.fromIBAN || !formData.toIBAN || !formData.amount) {
                showAlert("error", "Eksik Bilgi", "Zorunlu alanları doldurun.");
                return;
            }

            if (recipientName && !nameMatch) {
                showAlert("warning", "Doğrulama", "İsim doğrulaması başarısız.");
                return;
            }

            onSubmit(formData);
        };

        return (
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div>
                    <label className="flex items-center space-x-2 text-sm font-semibold text-gray-300 mb-2">
                        <CreditCard className="w-4 h-4 text-blue-400" />
                        <span>Kaynak Hesap</span>
                    </label>
                    <select
                        name="fromIBAN"
                        value={formData.fromIBAN}
                        onChange={handleAccountChange}
                        className="w-full bg-[#1e293b] border border-[#1e222d] text-white px-4 py-3 rounded-lg text-sm"
                    >
                        <option value="">Hesap Seçiniz</option>
                        {fromAccounts.map((acc) => (
                            <option key={acc.id} value={acc.iban}>
                                {acc.name} • {acc.iban.slice(-8)} • {currencySymbols[acc.currency]}{acc.balance.toLocaleString()}
                            </option>
                        ))}
                    </select>
                </div>

                <div>
                    <label className="flex items-center space-x-2 text-sm font-semibold text-gray-300 mb-2">
                        <Building2 className="w-4 h-4 text-blue-400" />
                        <span>Alıcı IBAN</span>
                    </label>
                    <input
                        name="toIBAN"
                        value={formData.toIBAN}
                        onChange={handleChange}
                        placeholder="TR..."
                        maxLength={32}
                        className="w-full bg-[#1e293b] border border-[#1e222d] text-white px-4 py-3 rounded-lg text-sm"
                    />
                    {ibanLoading && <p className="text-xs text-gray-400 mt-2 animate-pulse">Kontrol ediliyor...</p>}
                    {recipientName && (
                        <div className="mt-2 bg-[#111827]/70 border border-[#1e222d] rounded-lg p-2 text-xs text-gray-300">
                            Alıcı: <span className="text-blue-400 font-semibold">{maskName(recipientName)}</span>
                        </div>
                    )}
                </div>

                {recipientName && (
                    <div>
                        <label className="flex items-center space-x-2 text-sm font-semibold text-gray-300 mb-2">
                            <CheckCircle2 className="w-4 h-4 text-blue-400" />
                            <span>Alıcı İsmini Doğrula</span>
                        </label>
                        <input
                            name="recipientNameConfirm"
                            value={recipientNameConfirm}
                            onChange={handleChange}
                            placeholder="Ad Soyad"
                            className={`w-full bg-[#1e293b] border px-4 py-3 rounded-lg text-sm ${nameMatch === false ? 'border-red-600' : nameMatch ? 'border-green-600' : 'border-[#1e222d]'}`}
                        />
                        {nameMatch === false && <p className="text-red-500 text-xs mt-1">İsim uyuşmuyor.</p>}
                        {nameMatch && <p className="text-green-500 text-xs mt-1">Doğrulandı ✅</p>}
                    </div>
                )}

                <div>
                    <label className="flex items-center space-x-2 text-sm font-semibold text-gray-300 mb-2">
                        <DollarSign className="w-4 h-4 text-blue-400" />
                        <span>Tutar</span>
                    </label>
                    <div className="relative">
                        <input
                            type="number"
                            name="amount"
                            min="1"
                            step="0.01"
                            value={formData.amount}
                            onChange={handleChange}
                            placeholder="0.00"
                            className="w-full bg-[#1e293b] border border-[#1e222d] text-white px-4 py-3 rounded-lg text-sm"
                        />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">
                            {currentAccount ? currencySymbols[currentAccount.currency] : "₺"}
                        </span>
                    </div>
                </div>

                <div>
                    <label className="flex items-center space-x-2 text-sm font-semibold text-gray-300 mb-2">
                        <FileText className="w-4 h-4 text-blue-400" />
                        <span>Açıklama</span>
                    </label>
                    <textarea
                        name="description"
                        value={formData.description}
                        onChange={handleChange}
                        rows={3}
                        placeholder="Transfer açıklaması..."
                        className="w-full bg-[#1e293b] border border-[#1e222d] text-white px-4 py-3 rounded-lg text-sm resize-none"
                    />
                </div>

                <div className="flex gap-3 pt-2">
                    <button type="button" onClick={onClose} className="flex-1 bg-[#1e293b] hover:bg-[#334155] text-white px-6 py-3 rounded-lg text-sm">
                        İptal
                    </button>
                    <button
                        type="submit"
                        disabled={isLoading || (recipientName !== null && !nameMatch)}
                        className={`flex-1 flex justify-center items-center gap-2 px-6 py-3 rounded-lg text-sm ${
                            isLoading || (recipientName !== null && !nameMatch)
                                ? 'bg-gray-700 text-gray-400 cursor-not-allowed'
                                : 'bg-blue-600 hover:bg-blue-700 text-white'
                        }`}
                    >
                        {isLoading ? <RefreshCw className="animate-spin w-4 h-4" /> : <><Send className="w-4 h-4" /> Devam Et</>}
                    </button>
                </div>
            </form>
        );
    }