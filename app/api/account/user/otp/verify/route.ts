...
export async function POST(request: NextRequest) {
    ...
    console.log(`[OTP_VERIFY_PROXY][${requestId}] Request başladı.`);

    try {
        const { email, otp } = await request.json();

        if (!email || !otp) {
            console.log(`[OTP_VERIFY_PROXY][${requestId}] Validasyon hatası: Email veya OTP eksik.`);
        ...
        console.log(`[OTP_VERIFY_PROXY][${requestId}] Backend'e istek atılıyor: ${TARGET_URL}`);
        ...
        console.log(`[OTP_VERIFY_PROXY][${requestId}] Başarılı (${duration}ms).`);
        ...
        if (axios.isAxiosError(error)) {
            ...
            console.error(`[OTP_VERIFY_PROXY][${requestId}] Backend Hatası (${statusCode}):`, responseData);
            ...
            console.error(`[OTP_VERIFY_PROXY][${requestId}] Kritik Hata:`, error);
            errorResponse = {
                status: 'ERROR',
